...
class ONSTicketCreatorTool(CodedTool):
    ...
    def __init__(self):
        ...
        if self.mock_mode:
            logging.info("[ONSTicketCreatorTool] Running in MOCK mode - ServiceNow credentials not configured")
            self.api_url = None
        else:
    ...
    def invoke(self, args: Dict[str, Any], sly_data: Dict[str, Any]) -> Union[str, Dict[str, Any]]:
        ...
        if self.mock_mode:
            ...
            logging.info("[ONSTicketCreatorTool] MOCK MODE - Generated mock ticket: %s", mock_ticket)
            return mock_ticket
