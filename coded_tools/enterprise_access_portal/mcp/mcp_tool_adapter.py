...
class MCPToolAdapter(CodedTool):
    ...
    async def async_invoke(self, args: Dict[str, Any], sly_data: Dict[str, Any]) -> Any:
        ...
        try:
            ...
            # Build tool_params by EXPLICITLY extracting only the parameters needed
            # This follows the BmiCalculator pattern - don't iterate through args
            tool_params = {}

            if tool_name == "user_verification":
                tool_params["user_id"] = args.get("user_id") or sly_data.get("user_id", "")
                tool_params["dataiku_id"] = args.get("dataiku_id") or sly_data.get("dataiku_id", "")

            elif tool_name == "training_requirements":
                tool_params["env"] = args.get("env", "")

            elif tool_name == "training_completions":
                tool_params["user_id"] = args.get("user_id") or sly_data.get("user_id", "")
                tool_params["training_name"] = args.get("training_name", "")
                tool_params["certificate_id"] = args.get("certificate_id", "")

            elif tool_name == "approvals_required":
                tool_params["env"] = args.get("env", "")
                tool_params["access_type"] = args.get("access_type", "")

            elif tool_name == "prompt_retriever":
                tool_params["agent_name"] = args.get("agent_name", "")
                use_case = args.get("use_case")
                if use_case:
                    tool_params["use_case"] = use_case

            # Invoke the MCP tool
            logging.info(f"[MCPToolAdapter] Calling MCP tool with params: {tool_params}")
            result = await target_tool.ainvoke(tool_params)
            logging.info(f"[MCPToolAdapter] Tool '{tool_name}' returned: {type(result).__name__} = {result}")

            # Ensure result is serializable - convert to string if it's not a basic type
            if isinstance(result, (str, int, float, bool, type(None))):
                return result
            elif isinstance(result, (dict, list)):
                # Try to return as-is if it's a dict/list (should be serializable)
                return result
            else:
                # Convert complex objects to string to ensure serializability
                return str(result)
